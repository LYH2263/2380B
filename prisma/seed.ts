import { PrismaClient, AchievementCategory, ItemCategory } from '@prisma/client'
import bcrypt from 'bcryptjs'

const prisma = new PrismaClient()

function generateInviteCode(salt: string) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    seed = Math.floor(seed / chars.length) + 1337
  }
  return code
}
...
async function main() {
  ...
  const admin = await prisma.user.create({
    data: {
      ...
      inviteCode: generateInviteCode('admin@neurosama.fan'),
      points: 2500,
      totalPoints: 2500,
      level: 5
    }
  ...
  const user1 = await prisma.user.create({
    data: {
      ...
      inviteCode: generateInviteCode('neuro_fan@example.com'),
      points: 850,
      totalPoints: 850,
      level: 3
    }
  ...
  const user2 = await prisma.user.create({
    data: {
      ...
      inviteCode: generateInviteCode('vedal_lover@example.com'),
      points: 320,
      totalPoints: 320,
      level: 2
    }
...
这两个家伙，真的是让人又头疼又感到骄傲啊。`
  ...
  // ==================== 初始化标签 ====================
  const tagDefs = [
    { name: '日常', category: 'GENRE' as const, description: '以日常生活为主题的作品' },
    { name: '温馨', category: 'STYLE' as const, description: '温暖治愈的风格' },
    { name: '搞笑', category: 'STYLE' as const, description: '以幽默搞笑为主的风格' },
    { name: 'Neuro-sama', category: 'ELEMENT' as const, description: '包含 Neuro-sama 角色' },
    { name: 'Vedal', category: 'ELEMENT' as const, description: '包含 Vedal 角色' },
    { name: '科幻', category: 'GENRE' as const, description: '科幻题材作品' },
    { name: '冒险', category: 'GENRE' as const, description: '冒险题材作品' },
    { name: '太空', category: 'ELEMENT' as const, description: '太空相关元素' },
    { name: '热血', category: 'STYLE' as const, description: '热血激昂的风格' },
    { name: '喜剧', category: 'GENRE' as const, description: '喜剧题材作品' },
    { name: '美食', category: 'ELEMENT' as const, description: '美食相关元素' },
    { name: '沙雕', category: 'STYLE' as const, description: '搞笑逗比的风格' },
    { name: '偶像', category: 'GENRE' as const, description: '偶像题材作品' },
    { name: '励志', category: 'STYLE' as const, description: '励志成长的风格' },
    { name: '音乐', category: 'ELEMENT' as const, description: '音乐相关元素' },
    { name: 'Evil Neuro', category: 'ELEMENT' as const, description: '包含 Evil Neuro 角色' },
  ]
  ...
  for (const tagDef of tagDefs) {
    ...
    createdTags[tagDef.name] = tag.id
  }
  console.log(`✅ 初始化了 ${tagDefs.length} 个标签`)

  // 小说-标签关联
  const novelTagRelations = [
    { novelId: novel1.id, tags: ['日常', '温馨', '搞笑', 'Neuro-sama', 'Vedal'] },
    { novelId: novel2.id, tags: ['科幻', '冒险', '太空', 'Neuro-sama', 'Vedal', '热血'] },
    { novelId: novel3.id, tags: ['喜剧', '日常', '美食', 'Neuro-sama', '沙雕'] },
    { novelId: novel4.id, tags: ['偶像', '励志', '音乐', 'Neuro-sama', 'Evil Neuro', '热血'] },
  ]

  for (const relation of novelTagRelations) {
    for (const tagName of relation.tags) {
      const tagId = createdTags[tagName]
      if (tagId) {
        await prisma.novelTag.upsert({
          where: { novelId_tagId: { novelId: relation.novelId, tagId } },
          create: { novelId: relation.novelId, tagId },
          update: {},
        }).catch(() => {})
      }
    }
  ...
  // 更新标签使用计数
  for (const tagName of Object.keys(createdTags)) {
    const tagId = createdTags[tagName]
    const count = await prisma.novelTag.count({ where: { tagId } })
    await prisma.tag.update({
      where: { id: tagId },
      data: { useCount: count },
    })
  }
  console.log('✅ 创建了小说-标签关联')
  ...
  const achievementDefs = [
    { code: 'FIRST_LOGIN', name: '初来乍到', description: '首次登录平台', icon: '🎯', points: 10, category: AchievementCategory.GENERAL, criteria: JSON.stringify({ type: 'count', metric: 'loginCount', target: 1 }) },
    { code: 'CHECKIN_30', name: '坚持不懈', description: '累计签到30天', icon: '📅', points: 50, category: AchievementCategory.GENERAL, criteria: JSON.stringify({ type: 'count', metric: 'checkinCount', target: 30 }) },
    { code: 'CHECKIN_100', name: '忠实粉丝', description: '累计签到100天', icon: '🏆', points: 200, category: AchievementCategory.GENERAL, criteria: JSON.stringify({ type: 'count', metric: 'checkinCount', target: 100 }) },
    { code: 'CONSECUTIVE_CHECKIN_30', name: '铁签到人', description: '连续签到30天', icon: '🎖️', points: 150, category: AchievementCategory.GENERAL, criteria: JSON.stringify({ type: 'consecutive', metric: 'consecutiveCheckinDays', target: 30 }) },
    { code: 'FIRST_NOVEL', name: '笔耕初试', description: '发布第一部小说', icon: '✍️', points: 20, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'count', metric: 'novelCount', target: 1 }) },
    { code: 'NOVEL_5', name: '多产作家', description: '发布5部小说', icon: '📚', points: 100, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'count', metric: 'novelCount', target: 5 }) },
    { code: 'CHAPTER_10', name: '持之以恒', description: '发布10个章节', icon: '📖', points: 30, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'count', metric: 'chapterCount', target: 10 }) },
    { code: 'CHAPTER_100', name: '著作等身', description: '发布100个章节', icon: '📕', points: 150, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'count', metric: 'chapterCount', target: 100 }) },
    { code: 'CONSECUTIVE_UPDATE_7', name: '笔耕不辍', description: '连续7天更新章节', icon: '🔥', points: 80, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'consecutive', metric: 'consecutiveUpdateDays', target: 7 }) },
    { code: 'CONSECUTIVE_UPDATE_30', name: '日日精进', description: '连续30天更新章节', icon: '💎', points: 300, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'consecutive', metric: 'consecutiveUpdateDays', target: 30 }) },
    { code: 'NOVEL_100_FAVORITES', name: '小有名气', description: '作品获得100个收藏', icon: '💖', points: 50, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'threshold', metric: 'novelMaxFavorites', target: 100 }) },
    { code: 'NOVEL_1000_FAVORITES', name: '万人迷', description: '作品获得1000个收藏', icon: '👑', points: 200, category: AchievementCategory.CREATION, criteria: JSON.stringify({ type: 'threshold', metric: 'novelMaxFavorites', target: 1000 }) },
    { code: 'FIRST_COMMENT', name: '畅所欲言', description: '发布第一条评论', icon: '💬', points: 5, category: AchievementCategory.SOCIAL, criteria: JSON.stringify({ type: 'count', metric: 'commentCount', target: 1 }) },
    { code: 'COMMENT_100', name: '评论家', description: '发布100条评论', icon: '🗣️', points: 60, category: AchievementCategory.SOCIAL, criteria: JSON.stringify({ type: 'count', metric: 'commentCount', target: 100 }) },
    { code: 'RATING_10', name: '鉴赏家', description: '为10部作品评分', icon: '⭐', points: 20, category: AchievementCategory.SOCIAL, criteria: JSON.stringify({ type: 'count', metric: 'ratingCount', target: 10 }) },
    { code: 'INVITE_5', name: '友谊使者', description: '邀请5位新用户注册', icon: '🤝', points: 80, category: AchievementCategory.SOCIAL, criteria: JSON.stringify({ type: 'count', metric: 'inviteCount', target: 5 }) },
    { code: 'FAVORITE_10', name: '藏书家', description: '收藏10部作品', icon: '📋', points: 15, category: AchievementCategory.READING, criteria: JSON.stringify({ type: 'count', metric: 'favoriteCount', target: 10 }) },
    { code: 'READ_CHAPTER_50', name: '手不释卷', description: '阅读50个章节', icon: '📗', points: 40, category: AchievementCategory.READING, criteria: JSON.stringify({ type: 'count', metric: 'readChapterCount', target: 50 }) },
    { code: 'LEVEL_5', name: '小有成就', description: '用户等级达到5级', icon: '🌟', points: 100, category: AchievementCategory.SPECIAL, criteria: JSON.stringify({ type: 'threshold', metric: 'level', target: 5 }) },
    { code: 'LEVEL_10', name: '神话传说', description: '用户等级达到满级', icon: '🌈', points: 500, category: AchievementCategory.SPECIAL, criteria: JSON.stringify({ type: 'threshold', metric: 'level', target: 10 }) },
  ]
  ...
  console.log(`✅ 初始化了 ${achievementDefs.length} 个成就定义`)

  // ==================== 初始化商城物品 ====================
  const shopItems = [
    { name: '青铜头像框', description: '低调奢华的青铜边框，彰显沉稳气质', price: 100, category: ItemCategory.AVATAR_FRAME, effectValue: 'bronze', durationDays: 30, stock: null, soldCount: 0 },
    { name: '白银头像框', description: '闪耀银光的白银边框，低调中透着高贵', price: 300, category: ItemCategory.AVATAR_FRAME, effectValue: 'silver', durationDays: 30, stock: null, soldCount: 0 },
    { name: '黄金头像框', description: '璀璨夺目的黄金边框，作者身份的象征', price: 800, category: ItemCategory.AVATAR_FRAME, effectValue: 'gold', durationDays: 30, stock: null, soldCount: 0 },
    { name: '钻石头像框', description: '晶莹剔透的钻石边框，闪耀全场', price: 2000, category: ItemCategory.AVATAR_FRAME, effectValue: 'diamond', durationDays: 90, stock: 100, soldCount: 0 },
    { name: '彩虹头像框', description: '七彩流光的稀有边框，限时限量', price: 5000, category: ItemCategory.AVATAR_FRAME, effectValue: 'rainbow', durationDays: null, stock: 50, soldCount: 0 },

    { name: '深海蓝昵称色', description: '深邃如海洋般的蓝色昵称', price: 150, category: ItemCategory.NICKNAME_COLOR, effectValue: '#3b82f6', durationDays: 30, stock: null, soldCount: 0 },
    { name: '梦幻紫昵称色', description: '神秘优雅的紫色，高贵非凡', price: 250, category: ItemCategory.NICKNAME_COLOR, effectValue: '#a855f7', durationDays: 30, stock: null, soldCount: 0 },
    { name: '樱花粉昵称色', description: '少女心满满的粉色，甜度爆表', price: 250, category: ItemCategory.NICKNAME_COLOR, effectValue: '#ec4899', durationDays: 30, stock: null, soldCount: 0 },
    { name: '尊贵金昵称色', description: '金光灿灿的金色，尊贵无比', price: 600, category: ItemCategory.NICKNAME_COLOR, effectValue: '#eab308', durationDays: 60, stock: null, soldCount: 0 },

    { name: '月度免广告', description: '30天全站免广告，纯净阅读体验', price: 500, category: ItemCategory.AD_FREE, effectValue: 'adfree', durationDays: 30, stock: null, soldCount: 0 },
    { name: '季度免广告', description: '90天全站免广告，超值套餐', price: 1200, category: ItemCategory.AD_FREE, effectValue: 'adfree', durationDays: 90, stock: null, soldCount: 0 },
    { name: '永久免广告', description: '终身免除所有广告，至尊体验', price: 9999, category: ItemCategory.AD_FREE, effectValue: 'adfree_permanent', durationDays: null, stock: 200, soldCount: 0 },

    { name: '资深读者徽章', description: '专属身份徽章，展示于个人主页', price: 1500, category: ItemCategory.BADGE, effectValue: 'badge_reader', durationDays: null, stock: null, soldCount: 0 },
    { name: '核心粉丝徽章', description: 'Neurosama 核心粉丝的专属荣耀', price: 3000, category: ItemCategory.BADGE, effectValue: 'badge_corefan', durationDays: null, stock: 500, soldCount: 0 },
  ]
  ...
  console.log(`✅ 初始化了 ${shopItems.length} 个商城物品`)

  // ==================== 给种子用户授予基础成就 ====================
  const basicAchievements = ['FIRST_LOGIN']
  for (const userId of [admin.id, user1.id, user2.id]) {
    ...
    await prisma.pointRecord.create({
      data: {
        userId,
        points: userId === admin.id ? 2500 : userId === user1.id ? 850 : 320,
        source: 'SYSTEM',
        description: '初始积分',
      },
    })
  }
  console.log('✅ 给种子用户授予了基础成就和积分记录')

  // 给 user2 授予评论相关成就（他发了评论）
  const commentAch = await prisma.achievement.findUnique({ where: { code: 'FIRST_COMMENT' } })
  if (commentAch) {
    await prisma.userAchievement.create({
      data: { userId: user2.id, achievementId: commentAch.id },
    }).catch(() => {})
  }

  // 给 admin 授予创作相关成就
  const firstNovelAch = await prisma.achievement.findUnique({ where: { code: 'FIRST_NOVEL' } })
  if (firstNovelAch) {
    await prisma.userAchievement.create({
      data: { userId: admin.id, achievementId: firstNovelAch.id },
    }).catch(() => {})
  }
  ...
  console.log('👤 创建了 3 个用户（带邀请码/积分/等级）')
  console.log('📖 创建了多个章节')
  console.log('💬 创建了多条评论')
  console.log('🏷️  初始化了 ' + tagDefs.length + ' 个标签')
  console.log('🏆 初始化了 ' + achievementDefs.length + ' 个成就')
  console.log('🛒 初始化了 ' + shopItems.length + ' 个商城物品')
  console.log('----------------------------------------')
  console.log('🔑 测试账号:')
  console.log('   管理员: admin@neurosama.fan / admin123')
  console.log('     - 邀请码: ' + admin.inviteCode + ' | Lv.5 | 2500 积分')
  console.log('   用户1: neuro_fan@example.com / user123')
  console.log('     - 邀请码: ' + user1.inviteCode + ' | Lv.3 | 850 积分')
  console.log('   用户2: vedal_lover@example.com / user123')
  console.log('     - 邀请码: ' + user2.inviteCode + ' | Lv.2 | 320 积分')
  console.log('----------------------------------------')
}
